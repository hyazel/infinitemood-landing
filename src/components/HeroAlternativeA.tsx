...
import { motion, AnimatePresence, useMotionValue, useTransform, useMotionValueEvent, useMotionTemplate, animate, useScroll } from 'framer-motion';
...
const HeroAlternativeA: React.FC<HeroAlternativeAProps> = ({ onComplete }) => {
    ...
    // Start: Centered at 92% down. End: Centered at 50%.
    const straplineTop = useTransform(scrollYProgress, [0, 1], ["92%", "50%"]);
    ...
    // Unlock next sections when reaching end of scroll
    useMotionValueEvent(scrollYProgress, "change", (latest) => {
        if (latest > 0.9 && !hasTriggered) {
    ...
    const handleExplore = () => {
        // Force scroll to end if clicked
        const nextSection = document.getElementById('manifesto');
        if (nextSection) {
            nextSection.scrollIntoView({ behavior: 'smooth' });
        } else {
            // If manifesto not yet rendered, we assume we need to scroll to bottom of hero
